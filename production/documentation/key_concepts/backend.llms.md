...
An **openEO backend** is a server-side implementation of the openEO API. It exposes Earth Observation data, processing capabilities, authentication, and execution infrastructure through a standard interface, enabling the client-side workflow to run across different providers with minimal changes.

In practice, a backend is the system that receives your process graph, validates it, executes it near the data, and returns results or job metadata. This is what makes the openEO workflow interoperable.

![](images/backend-overview.png)

openEO backend overview
...
- **Data processing** through synchronous requests for smaller jobs and batch jobs for larger workloads with the available set of processes in the specific backend.
...
The authoritative live overview of publicly indexed openEO services is the [openEO Hub](https://hub.openeo.org/). The hub provides a detailed view on the backend metadata and lets you compare services based on what they expose.

This interface allows you to answer questions such as:

- What are the available backends?
- Which collections and processes are supported by each backend?
- What are the supported export formats and runtimes?

You can explore the live catalogue directly here:

Sorry, the [openEO](https://openeo.org/) Hub requires JavaScript to be enabled!

If the embedded view does not load in your browser, open the hub directly at <https://hub.openeo.org/>.

![](images/backend-services.png)

Backend service discovery example

> **NOTE:**
>
> The openEO hub data shown in the embedded view above is updated with each crawl, which may take a few days. For the most up-to-date information, always check the openEO hub directly.

## Choosing a backend

When selecting a backend for your work, start with these checks:

1.  What are the **collections** and **processes** you need for your workflow?
2.  Does it provide the **collections** you need?
3.  Does it support the **processes** and **execution mode** your workflow requires?
4.  Does it offer the right **export formats**, **authentication model**, and **quota or billing model**?
5.  Does it support advanced features you rely on, such as **UDFs**, **web services**, or **federated access**?

For a hands-on example of connecting to a backend and exploring its capabilities, see the [Setup](../../documentation/setup.llms.md) and [Cookbook](../../documentation/cookbook.llms.md) sections.
...
- the [openEO glossary](../../documentation/key_concepts/glossary.llms.md),
