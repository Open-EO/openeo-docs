...
before_cube = eoconn.load_collection(
    "SENTINEL2_L2A_SENTINELHUB", temporal_extent=before_date, bands=["B08", "B12"]
)
...
pre_nbr = before_cube.apply_polygon(geometries=spatial_param, process=udf_process)
post_nbr = after_cube.apply_polygon(geometries=spatial_param, process=udf_process)
```
