...
You can get it through CRAN: [openeo](https://cran.r-project.org/package=openeo) This means you can install or update the R client with the following command:
...
- Many functions such as `list_collections` , `describe_collection`, `list_processes` now return visual components in environments that support it (Jupyter, RMarkdown, knitr). Here’s an example of the collection list, which includes searching capabilities: ![](./images/r-client-vis.png)
