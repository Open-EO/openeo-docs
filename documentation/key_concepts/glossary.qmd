...
- **API**: Application Programming Interface ([wikipedia](https://en.wikipedia.org/wiki/Application_programming_interface){target="_blank"}),
 a (standardised) communication protocol, for example, between a client application and a backend service
...
It can be part of another process graph directly or stored as a custom process on the backend 
...
In the EO domain, it is common to be implicit about the temporal dimension and refer to them as spatial datacubes in short.
...
A **feature** is a thing that usually has a geometry (e.g. the outline of an agricultural field, a forest or an urban area) and it may have additional properties assigned (e.g. a name, a colour, or a population).
In rare cases, features may lack geometry, often referred to as an "empty geometry".
