...
![A part of our original image is displayed here, to observe the effect of applying a linear scale: Original (thus unscaled) imagery is seen on the left, and scaled values on the right.](../cookbook/urk_scaling.jpg){fig-alt="A comparison between an unscaled and a scaled raster is shown. Unscaled values are much larger, sclaed values much closer together and all below 255."}
...
![Image above: Example PNG output with the vegetation highlighted in red.](../cookbook/urk.jpg){fig-alt="Example PNG: false color composite highlighting vegetation in red."}
...
![This is a false color image of our area of interest for this chapter. The bands 4, 8 and 2 have been assigned to the RGB channels without any further processing. The surrounding area of the island Pellworm is shown at low tide.](../cookbook/pellworm_248.jpg){fig-alt="The AOI around Pellworm is shown as a false color RGB. RGB = red, nir and blue"}
...
::: {.callout-tip}
## Important: Check Band Indices and Bandnames!
...
![A correctly calculated NDVI would look as displayed here (scaled linearly).](../cookbook/pellworm_ndvi.jpg){fig-alt="NDVI image of the AOI"}
...
![Applying the above described threshold to the NDVI yields this result. Water and artificial surfaces are mostly masked from the image.](../cookbook/pellworm_threshold.jpg){fig-alt="The NDVI with a threshold of 0.3 applied"}
...
![Here we see the effect of transforming intensity data to db: Both parts of the image are linearly scaled, but only the lower part has been log-transformed. It makes the interpretation much easier because image pixels take on less 'extreme' values.](../cookbook/pellworm_s1.jpg){fig-alt="The AOI image is split into two parts: The upper part depicts SAR data with some very bright and many very dark pixels, the lower part is more balanced."}
