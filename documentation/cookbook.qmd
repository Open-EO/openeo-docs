...
### 2.5 Download quicklook

To create a PNG output, we need to scale the satellite data we have down to the 8bit range of a PNG image. For this, the scale range of our imagery has to be known. 
...
```python
# Scale pixel values to 8-bit range for PNG output
from openeo.processes import linear_scale_range

s2_png = s2_composite.apply(
    lambda x: linear_scale_range(x, 0, 2500, 0, 255)
)
s2_png.download("s2-rgb-quicklook.png")
```
...
rescaled <- p$apply(
  ...
  process = function(data, context) {
    p$linear_scale_range(x = data, inputMin = 0, inputMax = 2500, outputMin = 0, outputMax = 255)
  }
)

result_png <- p$save_result(
  data = rescaled,
  format = formats$output$PNG,
  options = list(red = "B04", green = "B03", blue = "B02")
)

job <- create_job(graph = result_png, title = "S2 RGB Quicklook PNG")
start_job(job = job)
```
...
```js
const { Formula } = require('@openeo/js-client');

var s2_scaled = builder.apply(s2_composite, new Formula("linear_scale_range(x, 0, 2500, 0, 255)"));

var result_png = builder.save_result(s2_scaled, "PNG", {
    red: "B04",
    green: "B03",
    blue: "B02"
});

await con.downloadResult(result_png, "s2-rgb-quicklook.png");
```
